...
// Function to generate a random price between 1 and 200
const generateRandomPrice = () => {
  return (Math.random() * (200 - 1) + 1).toFixed(2);
};

const Dashboard: React.FC<{ giftList: Gift[] }> = ({ giftList }) => {
  // Assign random prices to each gift
  const giftListWithRandomPrices = giftList.map((gift) => ({
    ...gift,
    price: generateRandomPrice(),
  }));

  // Calculate statistics
  const numberOfGifts = giftListWithRandomPrices.length;
  const giftsPerBrand = giftListWithRandomPrices.reduce<Record<string, number>>((acc, gift) => {
...
// Group the gifts into the specified price buckets and count the number of gifts in each bucket
  const histogramData = priceBuckets.map((bucket) => {
    const count = giftListWithRandomPrices.filter(
